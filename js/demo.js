 ...
 */
{
  ...
  // Hide nav and links when reading
  let scrollpos,
    sidelinks = document.querySelector(".frame__title-wrap"),
    navlinks = document.querySelector(".frame__links");

  const hideNavLinks = (scrollpos) => {
    scrollpos = window.scrollY;
    if (scrollpos > 200) {
      sidelinks.classList.remove('visible__framenavs');
      navlinks.classList.remove('visible__framenavs');

      sidelinks.classList.add('hidden__framenavs');
      navlinks.classList.add('hidden__framenavs');
    } else {
      sidelinks.classList.remove('hidden__framenavs');
      navlinks.classList.remove('hidden__framenavs');

      sidelinks.classList.add('visible__framenavs');
      navlinks.classList.add('visible__framenavs');
    }
    // console.log(navlinks.style.opacity = 255 - scrollpos);
  };
  window.addEventListener('scroll', hideNavLinks);
  // Custom mouse cursor.
  class CursorFx {
    constructor(el) {
      this.DOM = {
  ...
  class Slide {
    constructor(el) {
      ...
      this.DOM.showContentLink = this.DOM.title;
      ...
      this.DOM.showContentLink.addEventListener('click', this.onClickShowContentFn);
    }
    setCurrent() {
  ...
  const cursorFXonLink = link => {
    link.addEventListener('mouseenter', () => cursor.enter());
    link.addEventListener('mouseleave', () => cursor.leave());
    link.addEventListener('click', () => cursor.click());
  }
  // Custom cursor chnages state when hovering on elements with 'data-hover'.
  [...document.querySelectorAll('[data-hover]')].forEach(cursorFXonLink);
  [...document.querySelectorAll('a')].forEach(cursorFXonLink);
  [...document.querySelectorAll('.slide__title')].forEach(cursorFXonLink);
